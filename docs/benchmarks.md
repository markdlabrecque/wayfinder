...
|---|---|---|---|---|---|
| Resident memory, idle | ~1 GB | < 50 MB | 776.6 MB | 2145.8 MB | Solr: Docker container (`docker stats`). Wayfinder: native process (`ps -o rss=`). |
| Resident memory, 2M docs under query load | 2-4 GB | < 500 MB | 870.5 MB | 2153.6 MB | Solr: Docker container (`docker stats`). Wayfinder: native process (`ps -o rss=`). |
| Cold start to first query served | 10-30 s | < 1 s | 1.80 s | 0.26 s | Solr: Docker container (`docker run` to first successful ping). Wayfinder: native process (binary launch to first successful ping). |
| p95 query latency (facet+filter+highlight, 2000000 docs) | baseline | <= baseline | 9.78 ms | 13.97 ms | Solr: HTTP to the Docker container's published port. Wayfinder: HTTP to the native process's bound port. |
...
| Index size on disk | baseline | <= 1.2x baseline | 506.6 MB | 330.8 MB | Solr: size inside the Docker container's data volume (`docker exec du`). Wayfinder: size of the native process's data directory on the host (`du`). |

## Notes

- This is a long manual local run outside CI; indexing is expected to dominate wall time.
- Wayfinder missed the PRD's <500 MB query-load resident-memory target: 2145.8 MB was resident at the post-index idle sample, and RSS increased by 7.8 MB between that sample and the 2153.6 MB maximum sampled during query load.
