...
of 400ing. Recorded as finding 109 in `docs/solr-ref-findings.md`, appended after 108.
...
`ACCEPTED_DIVERGENCES` entry applies. Recorded in finding 109 and in the
...
2. **The Jetty-HTML `omitHeader` divergence is asserted in four places
   (finding 109, the `omit_header()` doc comment, this report, and the spec) with no
